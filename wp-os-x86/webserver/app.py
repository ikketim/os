#!/usr/bin/env python3
"""
WhiteoutProjectOS Web Control Panel
...
OS_USERNAME  = os.environ.get("OS_USERNAME",  "wp-os-user")
SERVICE_NAME = os.environ.get("SERVICE_NAME", "wosbot")
BOT_DIR      = os.environ.get("BOT_DIR",      "/home/wp-os-user/bot")
TOKEN_FILE   = os.environ.get("TOKEN_FILE",   "/home/wp-os-user/bot/bot_token.txt")
PORT         = int(os.environ.get("PORT",     "8080"))
SWITCH_SCRIPT = "/usr/local/bin/wp-os-switch-bot.sh"
BOT_TYPE_FILE = f"{BOT_DIR}/.bot_type"
JS_ENV_FILE   = f"{BOT_DIR}/src/.env"
DESKTOP_FLAG  = "/etc/wp-os/gui_enabled"

switch_lock   = threading.Lock()
switch_status = {"running": False, "log": [], "result": None}


def chown_to_user(path: str) -> None:
    """Hand ownership of a file to OS_USERNAME after root writes it."""
    try:
        pw = pwd.getpwnam(OS_USERNAME)
        os.chown(path, pw.pw_uid, pw.pw_gid)
    except Exception:
        pass  # non-fatal -- file is world-readable anyway
...
<header>
  <div class="logo">
    ...
    <div class="logo-text">WhiteoutProject<span>OS</span></div>
  </div>
  <div class="hdr-right">
...
def api_gui():
    ...
    try:
        Path("/etc/wp-os").mkdir(parents=True, exist_ok=True)
